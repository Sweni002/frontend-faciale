...
const handleCreatePersonnel = async () => {
  ...
  const formData = {
    ...
    nom,
    prenom ,
       email:tel, // si tu ne l'utilises pas pour l’instant
       iddiv: selectedDivision.iddiv,
    image: selectedImageURL // en base64
  };

  try {
    const response = await fetch("http://localhost:5000/api/responsables/", {
      method: "POST",
      headers: {
      ...
      <div className={styles.break}>
        <Breadcrumb
          ...
          items={[
            {
              title: (
                <a href="/global/responsable" style={{ fontSize: 17 }}>
                  Responsable
                </a>
              ),
            },
...
>
  <SnackbarContent
    ...
    message={<span style={{ marginRight: 8 }}>Responsable ajouté.</span>}
    action={action}
  />
</Snackbar>
